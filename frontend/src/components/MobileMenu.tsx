    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="w-[88vw] max-w-sm overflow-y-auto p-0">
        ...
        <nav className="flex flex-col p-2 text-sm">
          <SheetClose asChild>
            <Link to="/" className="px-3 py-2 rounded-md hover:bg-accent/10">Pacotes</Link>
          </SheetClose>
          <SheetClose asChild>
            <a href="#sobre" className="px-3 py-2 rounded-md hover:bg-accent/10">Sobre</a>
          </SheetClose>
          <SheetClose asChild>
            <a href="#contato" className="px-3 py-2 rounded-md hover:bg-accent/10">Contato</a>
          </SheetClose>
          ...
          {user ? (
            <>
              <button onClick={() => go("/conta")} className="flex items-center gap-2 px-3 py-2 rounded-md hover:bg-accent/10">
                <User className="h-4 w-4" /> Meus dados
              </button>
              {isAdmin && (
                <button onClick={() => go("/admin")} className="flex items-center gap-2 px-3 py-2 rounded-md hover:bg-accent/10">
        ...
        {user && (
          <div className="px-4 pb-4">
            <div className="flex items-center gap-2 mt-2 mb-3">
              ...
              <h3 className="font-semibold text-sm">Minhas reservas</h3>
            </div>

            {loading && <p className="text-xs text-muted-foreground">Carregando...</p>}
            {!loading && bookings && bookings.length === 0 && (
              <p className="text-xs text-muted-foreground">Você ainda não tem reservas.</p>
            )}
            {!loading && bookings && bookings.length > 0 && (
                  ...
                  return (
                    <li key={b.id} className="rounded-lg border p-3 text-xs space-y-1">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full border text-[10px] ${st.cls}`}>
                          {st.label}
                        </span>
                      ...
                      <div className="flex items-center justify-between text-muted-foreground">
                        ...
                        <span className="font-semibold text-foreground">
                          {Number(b.total_price).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                        </span>
                      </div>
                      {b.voucher_code && (
                        <div className="text-[10px] text-muted-foreground">Voucher: {b.voucher_code}</div>
                      )}
                    </li>
                  );
                })}
        ...
        {user && (
          <div className="p-4 border-t mt-2">
            <Button variant="outline" className="w-full" onClick={async () => { await signOut(); setOpen(false); nav("/"); }}>
