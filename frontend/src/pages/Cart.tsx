...
const Cart = () => {
  ...
  const reservar = async () => {
    ...
    for (const i of items) {
      try {
        await bookingsApi.create({ package_id: i.package_id, quantity: i.quantity });
        success++;
      } catch (e: unknown) {
        toast.error(`Erro em "${i.title}": ${e instanceof Error ? e.message : "Erro"}`);
      }
    }
    setBusy(false);
    if (success > 0) {
      toast.success(`${success} reserva(s) criada(s)! Acompanhe em "Minha conta".`);
      clear();
      nav("/conta");
    }
  };
                ...
                <Card key={i.package_id}>
                  <CardContent className="p-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                      <img src={i.cover_image || "/placeholder.svg"} alt={i.title} className="h-32 w-full rounded-lg object-cover sm:h-20 sm:w-28" />
                      ...
                      <div className="flex items-center justify-between gap-3 sm:gap-4">
                        <Input type="number" min={1} max={i.available_spots} value={i.quantity}
                          onChange={(e) => setQty(i.package_id, Number(e.target.value))} className="w-20 text-center" />
            ...
            <aside>
              <Card className="sticky top-24 shadow-elegant">
                <CardContent className="space-y-4 p-6">
                  ...
                  <Button onClick={reservar} disabled={busy} className="w-full bg-gradient-gold text-primary hover:opacity-90 shadow-gold">
                    {busy ? "Reservando..." : "Reservar pacotes"}
                  </Button>
                  <p className="text-xs text-center text-muted-foreground">
                    As reservas ficam <strong>pendentes</strong>. Em "Minha conta" você poderá acompanhar ou cancelar.
                  </p>
                </CardContent>
              </Card>
            </aside>
