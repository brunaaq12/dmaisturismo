...
import { packagesApi, bookingsApi, packageTypesApi, type Package } from "@/lib/api";
...
const PackageDetails = () => {
    ...
    packagesApi.get(id)
      .then(async (data) => {
        ...
        if (data.package_type) {
          const types = await packageTypesApi.list().catch(() => []);
          const t = types.find((x) => x.slug === data.package_type);
          if (t) setDiscountPct(Number(t.discount_percent || 0));
        }
        setLoading(false);
      })
      .catch(() => { toast.error("Pacote não encontrado"); nav("/"); });
  }, [id, nav]);
  ...
  const handleBook = async () => {
    ...
    for (const [i, p] of passengers.entries()) {
      if (!p.full_name.trim()) { toast.error(`Informe o nome do passageiro ${i + 2}`); return; }
      if (p.full_name.trim().length < 15) { toast.error(`Nome do passageiro ${i + 2} deve ter no mínimo 15 caracteres`); return; }
      if (!p.rg.trim()) { toast.error(`Informe o RG do passageiro ${i + 2}`); return; }
    }
    setBooking(true);
    try {
      const b = await bookingsApi.create({ package_id: pkg.id, quantity: qty, passengers });
      toast.success(`Reserva criada! Voucher: ${b.voucher_code}`);
  ...
  const finalPrice = pkg.price * qty * (qty > 1 ? 1 - discountPct / 100 : 1);
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {pkg.itinerary && (
            <section>
              ...
              <div className="rounded-xl border border-border bg-card p-5 text-sm leading-relaxed whitespace-pre-line">{pkg.itinerary}</div>
            </section>
          )}
          {pkg.included && (
            <section>
            ...
            <CardContent className="space-y-4">
              ...
              {passengers.length > 0 && (
                <div className="space-y-3 border-t border-border pt-4">
                  ...
                  <p className="text-xs text-muted-foreground">Você (titular) é o passageiro 1. Informe nome completo (mín. 15 caracteres) e RG (somente números) dos demais.</p>
                  {passengers.map((p, i) => (
                    <div key={i} className="rounded-lg border border-border p-3 space-y-2">
                      ...
                      <div>
                        ...
                        <Input id={`pname-${i}`} value={p.full_name} required minLength={15} placeholder="Mínimo 15 caracteres"
                          onChange={(e) => updatePassenger(i, "full_name", e.target.value)} />
                      </div>
                      <div>
                        <Label htmlFor={`prg-${i}`} className="text-xs">RG (somente números) *</Label>
                        <Input id={`prg-${i}`} value={p.rg} required inputMode="numeric" pattern="\d*" placeholder="Apenas dígitos"
              ...
              <div className="flex justify-between border-t border-border pt-4 font-semibold">
                <span>Total</span>
                <span className="font-serif text-xl text-primary">{formatBRL(finalPrice)}</span>
              </div>
              ...
              <Button variant="outline" disabled={soldOut} className="w-full"
                onClick={() => {
                  add({ package_id: pkg.id, title: pkg.title, cover_image: pkg.cover_image, price: Number(pkg.price), available_spots: pkg.available_spots }, qty);
                  toast.success("Adicionado ao carrinho");
                }}>
                Adicionar ao carrinho
              </Button>
              <p className="text-xs text-center text-muted-foreground">Sua reserva será enviada automaticamente para a equipe finalizar o pagamento.</p>
            </CardContent>
          </Card>
        </aside>
      </main>
