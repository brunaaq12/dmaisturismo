...
type BookingRow = {
  ...
  created_at: string;
  packages: { title: string | null; departure_date: string | null; location: string | null } | null;
};
...
const BookingsList = ({ rows, emptyMsg, onCancel }: { rows: BookingRow[]; emptyMsg: string; onCancel?: (id: string) => void }) => {
        ...
        return (
          <li key={b.id} className="rounded-lg border p-4 text-sm space-y-2">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              ...
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full border text-xs whitespace-nowrap ${st.cls}`}>
                {st.label}
              </span>
            </div>
...
const Account = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (userData) setProfile({ full_name: userData.full_name, phone: userData.phone });
        const bs = await api.get("/bookings");
  ...
  const saveProfile = async () => {
    ...
    if (!profile.phone || !/^\(?\d{2}\)?\s?9?\d{4}-?\d{4}$/.test(profile.phone.trim())) {
      toast.error("Contato inválido. Ex: (11) 91234-5678");
      return;
    }
    setSavingProfile(true);
    try {
      await api.put("/auth/profile", { full_name: profile.full_name, phone: profile.phone });
            ...
            <Card>
              ...
              <CardContent className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <Input value={profile.phone || ""} onChange={(e) => setProfile({ ...profile, phone: e.target.value })} placeholder="(11) 91234-5678" />
                </div>
            ...
            <Card>
              ...
              <CardContent>
                {loadingBookings
                  ...
                  : <BookingsList rows={active} emptyMsg="Você ainda não possui reservas." onCancel={async (id) => {
                      if (!confirm("Tem certeza que deseja cancelar esta reserva?")) return;
                      try {
                        await api.put(`/bookings/${id}`, { status: "cancelado", canceled_by: "user" });
